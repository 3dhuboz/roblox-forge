...
pub async fn send_chat_message(
    ...
    let api_key = state
        ...
        .clone()
        .ok_or("API key not set. Add OPENROUTER_API_KEY or ANTHROPIC_API_KEY to .env, or set it in Settings.")?;
...
pub async fn check_api_key(state: State<'_, AppState>) -> Result<Option<String>, String> {
    ...
    match key.as_ref() {
        ...
        Some(k) if k.starts_with("sk-ant-") => Ok(Some("anthropic".into())),
        Some(_) => Ok(Some("unknown".into())),
        None => Ok(None),
    }
}
